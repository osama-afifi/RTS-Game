...
public class AntiAircraft : Unit
{
	...
	protected float turretRotationSpeedFactor = 35.0f;
	float lastShotTime = 0.0f;
	protected float reloadTime = 5.0f;
	Vector3 targetPosition;
	bool dying = false;
	int antiTurretJamLimit = 10;
	int antiTurretJamFactor = 0;
	
	public AntiAircraft (GameObject Obj, Vector3 Pos)
        : base(Obj, Pos)
	{
		...
		attackableTypes = new List<unitType>() {unitType.Helicopter , unitType.Aircraft};
		//Intialize constants
		...
		firingState = false;
		type = unitType.AirDefense;
		antiTurretJamFactor = 0;
		collisionAvoidanceDistance = 5.0f;
	}
	...
	public override void attack ()
	{
		...
		if (targetUnit == null || targetUnit.unitObject == null || targetUnit.isDead() || attackableTypes.Contains(targetUnit.getUnitType())==false) {
			...
			} else
			{
				firingState  = true;
				antiTurretJamFactor++;
				//if(antiTurretJamFactor<antiTurretJamLimit)
				{
	...
	public override void fire ()
	{
		...
		if (firingState && lastShotTime > reloadTime) {
			WeaponManager.getInstance ().addFireMissile (new Stinger (turretBody.transform.position+turretBody.transform.forward*1.5f + Vector3.up*0.7f, ref targetUnit ));
			lastShotTime = 0;
		}
